...
def create_parallel_imgs(imgs, masks, flnames):
    ...
    return np.array(par_imgs_1), np.array(par_imgs_2), np.array(par_imgs_3), np.array(labels)
...
def get_unet(dropout_rate):
    ...
    glb_pool1 = GlobalMaxPooling2D()(pool1)
    if dropout_rate > 0.0:
        glb_pool1 = Dropout(dropout_rate)(glb_pool1)
    ...
    glb_pool2 = GlobalMaxPooling2D()(pool2)
    if dropout_rate > 0.0:
        glb_pool2 = Dropout(dropout_rate)(glb_pool2)

    #third paraller layer
    conv3 = Conv2D(128, (3, 3), padding='same')(input3)
    conv3 = ReLU()(conv3)
    conv3 = Conv2D(128, (3, 3), padding='same')(conv3)
    conv3 = ReLU()(conv3)
    pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)
    glb_pool3 = GlobalMaxPooling2D()(pool3)
    if dropout_rate > 0.0:
        glb_pool3 = Dropout(dropout_rate)(glb_pool3)
